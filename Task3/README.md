...
kubectl patch deployment jaeger-operator -n observability --type='json' -p='[{"op": "remove", "path": "/spec/template/spec/containers/1"}]'
...
kubectl exec -it $(kubectl get pods -l app=service-a -o jsonpath='{.items[0].metadata.name}') -- curl -s http://service-a:8080
```
